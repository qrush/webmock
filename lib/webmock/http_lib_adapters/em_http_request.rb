  ...
  module EventMachine
    ...
    class WebMockHttpRequest < EventMachine::HttpRequest
      ...
      def self.new(uri, options={})
        ...
        conn = WebMockHttpConnection.new(nil)
        conn.opts = req
        conn
      end

      class WebMockHttpConnection < EventMachine::HttpConnection
        def setup_request(method, options = {})
          client = WebMockHttpClient.new(self, HttpOptions.new(@opts.uri, options, method), options)
          callback { connection_completed }
          middleware.each do |m|
            client.callback &m.method(:response) if m.respond_to?(:response)
          end
          @clients.push client
      ...
      class WebMockHttpClient < EventMachine::HttpClient
        ...
        def send_request_with_webmock(head, body)
          ...
          if WebMock::StubRegistry.instance.registered_request?(request_signature)
            ...
            @conn.setup(make_raw_response(webmock_response), @uri,
              webmock_response.should_timeout ? "WebMock timeout error" : nil)
          elsif WebMock.net_connect_allowed?(request_signature.uri)
            raise "wtf dude"
            http = send_request_without_webmock(head, body)
            http.callback {
